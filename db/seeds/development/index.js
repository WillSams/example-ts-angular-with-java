...
const seedData = async (knex, tableName, data) => {
  if (tableName == 'room') await dropAllConstraints(knex, 'reservation');
...
const injectTables = async (knex, tableName) => {
  try {
    const data = await jsonReader(`./seeds/development/${tableName}s.json`, (error, data) => {
...
export const seed = async (knex) => {
  await injectTables(knex, 'room');
  await injectTables(knex, 'reservation');
};
